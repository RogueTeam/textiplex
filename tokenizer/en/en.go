...
// EnglishTokenizer lowers tokens and strips a small set of inflectional
// suffixes. Deliberately light and lossy, tuned for recall. Use a Snowball
// port if you need linguistic accuracy.
func EnglishTokenizer(in []byte) iter.Seq[*tokenizer.Token] {
	return tokenizer.TokenizeWithStemmer(in, EnglishStemmer)
}

func EnglishStemmer(raw []byte) ([]byte, bool) {
	n := len(raw)
	keep := n
	switch {
	...
	return tokenizer.FinishStemmer(raw, keep, englishFold)
}

func englishFold(b []byte) []byte {
