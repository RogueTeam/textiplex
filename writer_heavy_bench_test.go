package textiplex_test

import (
	"runtime"
	"strconv"
	"sync"
	"sync/atomic"
	"testing"

	"github.com/RogueTeam/textiplex"
	"github.com/RogueTeam/textiplex/fields"
	"github.com/RogueTeam/textiplex/pool"
	"github.com/RogueTeam/textiplex/storage"
	"github.com/RogueTeam/textiplex/testsuite"
	"github.com/RogueTeam/textiplex/testsuite/wikipedia"
	"github.com/RogueTeam/textiplex/tokenizer/en"
	"github.com/stretchr/testify/assert"
)

var DefaultTokenizer = en.TokenizerWithoutStopwords

const MaxBatchSize = 1024 * 1024 * 1024 // 1GB

func BenchmarkHeavyWriter(b *testing.B) {
	b.StopTimer()

	var writer = textiplex.Writer{
		TemporaryDirectory: testsuite.TempDirectory(b, "temp-*"),
		Directory:          testsuite.TempDirectory(b, "segments-*"),
	}

	b.ResetTimer()
	b.ReportAllocs()
	b.StartTimer()

	b.Run("Indexing", func(b *testing.B) {
		b.StopTimer()

		maxWorkers := runtime.NumCPU()

		batchPool := sync.Pool{
			New: func() any {
				return fields.NewBatch(1_000)
			},
		}

		assertions := assert.New(b)

		pages, err := wikipedia.Pages()
		if !assertions.NoError(err, "failed to prepare pages reader") {
			return
		}

		var pagesCh = make(chan *wikipedia.Page, maxWorkers)
		go func() {
			defer close(pagesCh)

			for page := range pages {
				pagesCh <- page
			}
		}()

		b.ResetTimer()
		b.ReportAllocs()
		b.StartTimer()
		var batchsCh = make(chan *fields.Batch, maxWorkers)

		go func() {
			defer close(batchsCh)

			fieldsPool := pool.New[storage.FieldDefinition](200)
			tokenPool := pool.New[storage.TokenDefinition](200)

			batch := batchPool.Get().(*fields.Batch)
			batch.Reset()
			var batchCount uint64

			var totalCount atomic.Int64
			var wg sync.WaitGroup
			for page := range pagesCh {
				titleField := fieldsPool.Get()
				contentField := fieldsPool.Get()

				totalFieldSize := fields.TextField(titleField, tokenPool, "title", page.Title, DefaultTokenizer)
				totalFieldSize += fields.TextField(contentField, tokenPool, "content", page.Content, DefaultTokenizer)

				batch.Insert(
					storage.DocumentId{Value: storage.RawValueFrom(strconv.AppendInt(nil, page.Id, 10))},
					totalFieldSize,
					titleField,
					contentField,
				)
				batchCount++
				oldTotalCount := totalCount.Add(1)

				if batch.Size >= MaxBatchSize {
					b.Logf("Prepared batch - document count: %d - total: %d", batchCount, oldTotalCount)
					batchsCh <- batch
					batch = batchPool.Get().(*fields.Batch)
					batch.Reset()
					batchCount = 0
				}

			}

			if batch.Size > 0 {
				b.Logf("Prepared final batch - document count: %d - total: %d", batchCount, totalCount.Load())
				batchsCh <- batch
			}

			wg.Wait()
		}()

		var errorsCh = make(chan error)
		go func() {
			var wg sync.WaitGroup

			for batch := range batchsCh {
				wg.Go(func() {
					defer func() {
						batchPool.Put(batch)
					}()

					b.Logf("Inserting batch of size %d", batch.Size)
					err := writer.Batch(batch)
					if err != nil {
						errorsCh <- err
					}
				})
			}

			wg.Wait()
			close(errorsCh)
		}()

		var allErrors []error
		for err := range errorsCh {
			allErrors = append(allErrors, err)
		}

		if !assertions.Len(allErrors, 0, "expecting no errors") {
			return
		}
	})

	runtime.GC()

	b.Run("Merge", func(b *testing.B) {
		b.StopTimer()

		assertions := assert.New(b)

		b.ResetTimer()
		b.ReportAllocs()
		b.StartTimer()

		err := writer.Merge()
		if !assertions.Nil(err, "failed to merge") {
			return
		}
	})
}
